...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    <DialogOverlay />
    {/* Centering via flex, not translate(-50%,-50%): that trick shares CSS
        transform variables with the slide-in/out animation utilities below
        and can resolve to an identity transform once the animation ends
        (e.g. under prefers-reduced-motion's near-zero animation duration). */}
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <DialogPrimitive.Content
        ref={ref}
        className={cn(
          'relative grid w-full max-w-lg max-h-[90vh] overflow-y-auto sm:max-h-[85vh] gap-4 border bg-background p-6 shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 sm:rounded-xl',
          className
        )}
        {...props}
      >
