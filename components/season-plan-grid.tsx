...
export default function SeasonPlanGrid({
  ...
  if (loading) {
    return (
      <Card>
        <CardContent className="p-12 text-center text-gray-500">Lade Stundenplan…</CardContent>
      </Card>
    );
  }
        ...
        <Card>
          <CardContent className="p-0 overflow-x-auto">
            <div className="min-w-[800px]">
              ...
              >
                {/* Corner cell */}
                <div className="h-10 border-b border-r border-gray-200 bg-gray-50 flex items-center px-3">
                  <span className="text-xs font-semibold text-gray-500">Platz / Tag</span>
                </div>
                {/* Day headers */}
                {DAYS.map((day) => (
                  <div
                    key={day}
                    className="h-10 border-b border-r border-gray-200 bg-gray-50 flex items-center justify-center"
                  >
                ...
                {/* Court rows */}
                {courts.length === 0 ? (
                  <div className="col-span-8 h-20 flex items-center justify-center text-sm text-gray-400">
                  ...
                  courts.map((court) =>
                    [0, 1, 2, 3, 4, 5, 6].map((day) => {
                      ...
                      return (
                        <div
                          key={`${court.id}-${day}`}
                          className="min-h-[60px] border-b border-r border-gray-100 p-1 space-y-1"
                        >
                          {day === 0 && (
          ...
          {[0, 1, 2, 3, 4, 5, 6].map((day) => {
            ...
            return (
              <Card key={day}>
                ...
                <CardContent>
                  {daySlots.length === 0 ? (
                    <p className="text-sm text-gray-400 py-4 text-center">Keine Einheiten</p>
                  ) : (
      ...
      {slots.length === 0 && (
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <p className="text-gray-500 font-medium">Keine geplanten Einheiten</p>
