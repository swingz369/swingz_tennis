  ...
  return (
    <div className="overflow-x-auto">
      <div className="min-w-[520px]">
        ...
        >
          ...
          {grid.hours.map((h) => (
            <span
              key={`h-${h}`}
              aria-hidden="true"
              className="grid place-items-center tabular-nums text-3xs text-muted-foreground"
            >
              {h}
            </span>
          ))}
        ...
        <div className="mt-3 flex items-center gap-1.5 tabular-nums text-2xs text-muted-foreground">
          <span>frei</span>
          {LEVELS.map((cls, i) => (
...
function Row({ label, cells, hours }: { label: string; cells: number[]; hours: number[] }) {
  return (
    <>
      <span className="grid place-items-center tabular-nums text-3xs text-muted-foreground">
        {label}
      </span>
      {cells.map((ratio, i) => (
        <span
