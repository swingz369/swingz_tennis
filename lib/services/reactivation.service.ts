...
export function shouldReactivate(member: InactiveMember, now: Date = new Date()): boolean {
  ...
  if (member.daysSinceLastBooking !== null && member.daysSinceLastBooking < INACTIVITY_THRESHOLD_DAYS) {
    return false;
  }
...
export class ReactivationService {
      ...
      try {
        ...
        if (sendResult.sent > 0) {
          ...
          if (updateError) {
            ...
            result.failed++;
            result.errors.push({ userId: member.userId, error: `markSent: ${updateError.message}` });
          } else {
            result.sent++;
  ...
  private static async findInactiveMembers(
    ...
    const { data: memberships, error: membershipsError } = await supabase
      .from('user_club_memberships')
      .select(
        'user_id, club_id, last_reactivation_sent_at, reactivation_count, users(full_name)'
      )
      .eq('is_active', true)
