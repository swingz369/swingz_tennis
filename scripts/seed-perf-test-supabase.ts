#!/usr/bin/env npx tsx
/**
 * seed-perf-test-supabase.ts
 *
 * Fix für den Auth-Hash-Bug in seed-perf-test.ts:
 * Statt User direkt per SQL in `auth.users` mit scrypt-Hash anzulegen,
 * nutzen wir `supabase.auth.admin.createUser()` (Service-Role-Key) —
 * so landen die Passwörter mit **korrekten bcrypt-Hashes** in auth.users
 * und sind via `/api/auth/login` (Supabase Auth API) verifizierbar.
 *
 * Voraussetzungen:
 *   - .env.local enthält:
 *       NEXT_PUBLIC_SUPABASE_URL=https://...
 *       SUPABASE_SERVICE_ROLE_KEY=eyJ...
 *   - Supabase-Projekt (lokal via `supabase start` ODER remote) ist erreichbar
 *
 * Usage:
 *   npx tsx scripts/seed-perf-test-supabase.ts
 *
 * Zweck: E2E-Performance-Tests (Playwright clustering-performance.spec.ts)
 *        brauchen einen Login-fähigen Admin (TestAdmin2026!).
 *
...
if (!DATABASE_URL) {
  console.error('❌ DATABASE_URL muss in .env.local gesetzt sein (für direkten SQL-Zugriff).');
  process.exit(1);
}

// ═══ NAME POOLS (identisch zu seed-perf-test.ts) ══════════════════════════
const FIRST_NAMES = [
  'Lukas',
  'Felix',
  'Maximilian',
...
/**
 * Idempotent helper: creates auth user via Admin API (proper bcrypt hash).
 * Returns the public.users id of the created/found user.
 */
async function ensureSupabaseUser(
  ...
  // Cheap pre-check: does public.users already have this email?
  ...
  // Create via Supabase Admin API (bcrypt hash → GoTrue kann's verifizieren)
  const { data, error } = await supabaseAdmin.auth.admin.createUser({
    email,
    password,
    email_confirm: true, // Skip email confirmation flow (perf seed)
    user_metadata: { full_name: fullName, ...extras },
  });
  ...
  // Insert into public.users (idempotent — trigger may have done this already)
  await pool.query(
    `INSERT INTO public.users (id, email, full_name, subscription_tier, subscription_status, created_at, updated_at${Object.keys(extras).length ? ', ' + Object.keys(extras).join(', ') : ''})
    ...
    [userId, email, fullName, ...Object.values(extras)]
  );
...
async function main() {
  ...
  console.log('🎾 Performance-Test-Seed (Supabase Admin) startet...\n');
  ...
  try {
    ...
    // ── 3. Admin user (via Supabase Auth Admin API) ────────────
    console.log('\n👤 Schritt 3: Admin-User (über supabase.auth.admin.createUser)...');
    const adminUserId = await ensureSupabaseUser(
      pool,
      ADMIN_EMAIL,
      ADMIN_PASSWORD,
      'PerfTest Admin'
    ...
    console.log(`     → login via /api/auth/login funktioniert (bcrypt-Hash korrekt)`);

    // ── 4. Trainer (8) ─────────────────────────────────────────
    console.log(`\n👨‍🏫 Schritt 4: ${NUM_TRAINERS} Trainer (über supabase.auth.admin.createUser)...`);
    ...
    for (let i = 0; i < NUM_TRAINERS; i++) {
      ...
      const userId = await ensureSupabaseUser(pool, email, TRAINER_PASSWORD, fullName, {
    ...
    // ── 6. Members (200) ───────────────────────────────────────
    console.log(
      `\n👥 Schritt 6: ${NUM_MEMBERS} Mitglieder erstellen (über supabase.auth.admin.createUser)...`
    );
    ...
    for (let i = 0; i < memberNames.length; i++) {
      ...
      try {
        const userId = await ensureSupabaseUser(pool, email, MEMBER_PASSWORD, fullName, {
          skill_level: level,
