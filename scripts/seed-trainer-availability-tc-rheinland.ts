...
const T1: TrainerProfile = {
  ...
  schedule: {
    1: [{ start: '09:00', end: '12:00' }, { start: '16:00', end: '21:00' }],
    2: [{ start: '09:00', end: '12:00' }, { start: '16:00', end: '21:00' }],
    3: [{ start: '09:00', end: '12:00' }, { start: '16:00', end: '21:00' }],
    4: [{ start: '09:00', end: '12:00' }, { start: '16:00', end: '21:00' }],
    5: [{ start: '09:00', end: '12:00' }, { start: '16:00', end: '21:00' }],
...
const stats: Stats = { availInserted: 0, availSkippedNoUser: 0, prefInserted: 0, prefSkipped: 0, errors: 0 };
...
async function main() {
  ...
  if (missing.length > 0) console.warn(`  ⚠️ Fehlend: ${missing.join(', ')} – werden übersprungen.`);
  ...
  for (const profile of ALL_TRAINERS) {
    ...
    if (inserted === rows.length) {
      ...
      log('✅', `${profile.fullName} (${profile.shortLabel}): +${inserted}/${rows.length} neu (${skipped} schon da)`);
    }
  }
  ...
  for (const profile of ALL_TRAINERS) {
    ...
    const { error } = await sb.from('user_training_preferences').insert(insertRow).select('id').single();
