...
- **Empfehlung:** Verifikation via `grep -rn 'critters' --include='*.ts'` — wenn 0, dann `npm uninstall`. _(↑ konsolidiert in BLOCKER B3)_
...
```json
{
  ...
  "ignoreMatches": ["knip", "depcheck", "lint-staged", "postcss", "autoprefixer", "@types/express"]
}
```
...
```json
{
  "quiet": true,
  "reporter": ["list"],
  "entry": ["app/layout.tsx", "app/page.tsx", "scripts/**", "e2e/**"],
