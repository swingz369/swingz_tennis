...
## `perf-bench` Workflow (Vitest-Bench → Report)

Advisorisches Benchmark auf jedem PR (und Push auf `main`), das die
Engine-Performance misst und einen Markdown-Report generiert.

### Pipeline

```
1. npx vitest bench tests/bench/clustering.bench.ts
     → tests/bench/.bench-results.json   (gitignored)
2. npx tsx scripts/generate-perf-report.ts
     → docs/PERFORMANCE_BENCHMARK.md     (gitignored)
3. Upload beider Dateien + logs als Artifact (Retention 90 Tage)
```

Beide Schritte laufen mit `continue-on-error: true` — Bench-Messungen sind
auf shared CI-Runnern anfällig für GC-Pauses / Throttling, also soll eine
Regression den PR nicht blockieren. Sichtbar bleibt sie trotzdem in der
PR-Checkliste und über die hochgeladenen Artifacts.

### Triggers

- `pull_request` (opened / synchronize / reopened) auf `main`, gefiltert auf
  `lib/season-planning/**`, `tests/bench/**`, `scripts/generate-perf-report.ts`,
  `.github/workflows/perf-bench.yml`
- `push` auf `main` mit denselben Pfadfiltern
- `workflow_dispatch` (manuell via `gh workflow run perf-bench.yml`)

### Perf-History-Integration

Die hochgeladenen JSON-Artefakte werden vom
`app/api/admin/perf-history/github`-Endpoint ausgelesen
(→ `lib/perf-history/github-source.ts`). Damit ergänzt der `perf-bench`-Workflow
den bereits bestehenden `local`-Endpoint (der `tests/bench/.bench-results.json`
auf dem Server liest) — Reviewer sehen den Trend im perf-history-Dashboard.

### Lokales Replay

```bash
npm run bench:full   # bench + report in einem Schritt
```

### Verwandte Workflows

- `.github/workflows/db-audit.yml` — Forensic DB-Audit (täglich/wochentlich)
- `.lighthouserc.json` — Lighthouse-CI (Frontend-Performance, on-demand)
