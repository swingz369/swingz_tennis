...
export default async function MembersPage({
  ...
  let query = (supabase.from('user_club_memberships') as any)
    .select(
      'id, user_id, role, is_active, is_honorary, honorary_since, joined_at, include_in_planning, member_number'
    )
    .eq('club_id', clubId)
  ...
  if (search && /^\d+$/.test(search.trim())) {
    // Reine Ziffern = Mitgliedsnummer. Führende Nullen sind Anzeigeformat
    // (formatMemberNumber), gespeichert ist die blanke Zahl — "0042" muss finden.
    const memberNumber = Number(search.trim());
    query = query.eq('member_number', memberNumber);
    countQuery = countQuery.eq('member_number', memberNumber);
  } else if (search) {
