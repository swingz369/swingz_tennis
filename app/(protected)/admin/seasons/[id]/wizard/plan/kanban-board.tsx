...
export function KanbanBoard({ seasonId, groups, allMembers }: {
  seasonId: string
  clubId?: string
  groups: Group[]
  allMembers: Member[]
  ...
  async function handleDragEnd(event: DragEndEvent) {
    ...
    try {
      if (sourceGroupId) await patchGroup(sourceGroupId, (groupMemberIds[sourceGroupId] ?? []).filter(id => id !== memberId))
      if (targetGroupId) await patchGroup(targetGroupId, [...(groupMemberIds[targetGroupId] ?? []), memberId])
    } catch {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <span className="text-sm text-muted-foreground ml-auto">{assignedCount}/{allMembers.length} eingeplant</span>
      </div>
