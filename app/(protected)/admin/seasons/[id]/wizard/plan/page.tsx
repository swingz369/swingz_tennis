...
export default async function PlanPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: season } = await (supabase as any).from('seasons').select('id, club_id').eq('id', id).single()
  if (!season) notFound()

  const [{ data: groups }, { data: memberships }] = await Promise.all([
    (supabase as any).from('groups').select('id, name, level, member_ids').eq('club_id', season.club_id).eq('is_active', true),
    (supabase as any)
  ...
  const members = (memberships ?? []).map((m: { user_id: string; users: { id: string; full_name: string | null; skill_level: string | null } }) => ({
    id: m.users.id,
