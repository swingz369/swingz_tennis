...
import { withCache, CACHE_PRESETS, PerformanceMonitor } from '@/lib/performance';
...
export async function GET(request: NextRequest) {
  return withAuth(request, async (auth) => {
    ...
    try {
      // Use caching for tenant overview (5 minutes)
      const result = await withCache(
        async () => {
          return PerformanceMonitor.measure('tenant-overview', async () => {
            ...
            return { clubs: clubsWithStats };
          });
        },
        {
          tags: ['tenants', 'clubs'],
          revalidate: CACHE_PRESETS.medium.revalidate,
          key: 'tenant-overview',
        }
      );
