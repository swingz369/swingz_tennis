...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  return withApiAuth(request, async (auth) => {
    ...
    const [{ data: entries, error: eErr }, { data: feeConfigs, error: fErr }] = await Promise.all([
      (auth.supabase as any).from('season_plan_entries').select('member_id, group_id').eq('season_id', seasonId),
      (auth.supabase as any).from('fee_configurations').select('*').eq('club_id', auth.clubId).eq('is_active', true),
    ])
    if (eErr) return NextResponse.json({ error: eErr.message }, { status: 500 })
    if (fErr) return NextResponse.json({ error: fErr.message }, { status: 500 })

    const preview = ((entries as any[]) ?? []).map((entry: any) => {
      const fee = ((feeConfigs as any[]) ?? []).find((f: any) => {
