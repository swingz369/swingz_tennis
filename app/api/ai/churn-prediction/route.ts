...
export async function GET() {
  try {
    ...
    const { data: attendanceRecords } = await supabase
      ...
      .gte('created_at', thirtyDaysAgo.toISOString()) as { data: Pick<AttendanceRow, 'participant_id'>[] | null };

    const attendanceCount = new Map<string, number>();
    for (const record of attendanceRecords ?? []) {
      attendanceCount.set(record.participant_id, (attendanceCount.get(record.participant_id) ?? 0) + 1);
    }
    ...
    const { data: recentBookings } = await supabase
      ...
      .gte('booked_at', thirtyDaysAgo.toISOString()) as { data: Pick<BookingRow, 'member_id' | 'booked_at'>[] | null };

    const recentBookingCount = new Map<string, number>();
    for (const booking of recentBookings ?? []) {
      recentBookingCount.set(booking.member_id, (recentBookingCount.get(booking.member_id) ?? 0) + 1);
    }
    ...
    const { data: olderBookings } = await supabase
      ...
      .gte('booked_at', sixtyDaysAgo.toISOString()) as { data: Pick<BookingRow, 'member_id'>[] | null };

    const olderBookingCount = new Map<string, number>();
    for (const booking of olderBookings ?? []) {
      olderBookingCount.set(booking.member_id, (olderBookingCount.get(booking.member_id) ?? 0) + 1);
    }
    ...
    for (const m of members) {
      ...
      if (bookingDecline) {
        ...
        // No booking history at all in last 60 days – add partial risk
        riskScore += 10;
