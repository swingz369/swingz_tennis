  ...
  try {
    ...
    if (!member_id) {
      return NextResponse.json(
        { error: 'Mitglied-ID ist erforderlich' },
        { status: 400 }
      );
    }
    ...
    if (!due_date) {
      return NextResponse.json(
        { error: 'Fälligkeitsdatum ist erforderlich' },
        { status: 400 }
      );
    }
    ...
    const createInvoiceData: CreateInvoice = {
      ...
      items: validItems.map((item: any) => ({
        ...
        item_type: ['membership_fee', 'training_fee', 'court_fee', 'dunning_fee', 'other'].includes(item.itemType)
          ? item.itemType
          : 'other',
      })),
