...
// forked from design-sync lib/source-kit.mjs — components/ui/*.tsx star-exports
// next/link-importing files (page-header.tsx etc.); their module-level Next.js
// internals (process.env.__NEXT_*) throw at bundle load in plain esbuild,
// breaking every preview. No cfg knob excludes files from the synth-entry scan
// (componentSrcMap only hides catalog entries), so this fork adds cfg.srcExclude.
