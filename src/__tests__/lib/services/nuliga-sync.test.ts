...
describe('resolveOwnTeam', () => {
  ...
  it('nimmt einen own_team_name, der nicht in der Tabelle steht, nicht als Treffer', () => {
    expect(resolveOwnTeam(standings, 'Herren 40')).toBeNull();
  });
...
describe('Spieler-Zuordnung', () => {
  ...
  it('ordnet über die DTB-ID zu', async () => {
    const { matchByDtbId } = await import('@/lib/services/nuliga-sync');
  ...
  it('schlägt über den Namen vor, wenn er eindeutig ist', async () => {
    const { suggestByName } = await import('@/lib/services/nuliga-sync');
