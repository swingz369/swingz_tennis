...
describe('reactivation.service — buildReactivationPayload', () => {
  ...
  it('falls back to threshold when daysSinceLastBooking is null', () => {
    const payload = buildReactivationPayload(makeMember({ daysSinceLastBooking: null }), '');
...
function setupMocks(
  opts: {
    ...
    pushResults?: Record<
      string,
      { success: boolean; sent: number; failed: number; cleanedUp: number }
    >;
    markError?: any;
  } = {}
): MockSetup {
      ...
      return {
        ...
        update: vi.fn((payload: any) => {
          ...
          chain.then = (onFulfilled: any, onRejected: any) => {
            ...
            return Promise.resolve({ data: null, error: markError }).then(onFulfilled, onRejected);
          };
          return chain;
        }),
      };
...
describe('ReactivationService.runReactivation', () => {
  ...
  it('per-member try/catch isolation: one member throws, others succeed', async () => {
    const { updateCalls } = setupMocks({
      ...
      bookings: [makeBooking('u1', 20), makeBooking('u2', 20), makeBooking('u3', 20)],
