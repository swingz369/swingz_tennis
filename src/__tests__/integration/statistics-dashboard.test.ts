...
// ── Mock hours-log adapter (always uses Drizzle repo) ──
vi.mock('@/src/application/services/hours-log-service.adapter', () => {
  const now = Date.now();
  ...
  return {
    hoursLogService: {
      getAllHoursLogs: vi.fn().mockResolvedValue(hoursLogs),
      getHoursLogsByTrainerId: vi.fn().mockResolvedValue([]),
      getHoursLogsByDateRange: vi.fn().mockResolvedValue([]),
      getHoursLogsByStatus: vi.fn().mockResolvedValue([]),
      createHoursLog: vi.fn(),
      getHoursLogById: vi.fn(),
      updateHoursLog: vi.fn(),
      approveHoursLog: vi.fn(),
      rejectHoursLog: vi.fn(),
      deleteHoursLog: vi.fn(),
      getHoursSummaryForTrainer: vi.fn(),
      getAllHoursSummaries: vi.fn(),
      getAllAttendanceRecords: vi.fn().mockResolvedValue([]),
      createAttendanceRecord: vi.fn(),
      getAttendanceRecordById: vi.fn(),
      getAttendanceRecordsBySessionId: vi.fn(),
      getAttendanceRecordsByTrainerId: vi.fn(),
      getAttendanceRecordsByParticipantId: vi.fn(),
      getAttendanceRecordsByDateRange: vi.fn(),
