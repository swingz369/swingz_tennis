import { Statistics, MemberStatistics, RevenueStatistics, CourtStatistics, TrainerStatistics, DashboardMetric } from '../../domain/entities/statistics.entity';
...
export class StatisticsService {
  ...
  async generateStatistics(period: 'daily' | 'weekly' | 'monthly' | 'yearly', startDate: Date, endDate: Date): Promise<Statistics> {
  ...
  async calculateTrainerStatistics(startDate: Date, endDate: Date): Promise<TrainerStatistics> {
    ...
    const sessionsByTrainer = filteredLogs.reduce((acc, log) => {
  ...
  async getDashboardMetrics(): Promise<DashboardMetric[]> {
    ...
    const lastMonthStats = await this.generateStatistics('monthly', startOfLastMonth, endOfLastMonth);
    ...
    return [
      {
        ...
        change: this.calculateChange(currentStats.memberStats.totalMembers, lastMonthStats.memberStats.totalMembers),
        changeType: this.calculateChangeType(currentStats.memberStats.totalMembers, lastMonthStats.memberStats.totalMembers),
        unit: 'members',
      ...
      },
      {
        ...
        change: this.calculateChange(currentStats.memberStats.activeMembers, lastMonthStats.memberStats.activeMembers),
        changeType: this.calculateChangeType(currentStats.memberStats.activeMembers, lastMonthStats.memberStats.activeMembers),
        unit: 'members',
      ...
      },
      {
        ...
        change: this.calculateChange(currentStats.revenueStats.totalRevenue, lastMonthStats.revenueStats.totalRevenue),
        changeType: this.calculateChangeType(currentStats.revenueStats.totalRevenue, lastMonthStats.revenueStats.totalRevenue),
        unit: '€',
      ...
      },
      {
        ...
        change: this.calculateChange(currentStats.courtStats.utilizationRate, lastMonthStats.courtStats.utilizationRate),
        changeType: this.calculateChangeType(currentStats.courtStats.utilizationRate, lastMonthStats.courtStats.utilizationRate),
        unit: '%',
      ...
      },
      {
        ...
        change: this.calculateChange(currentStats.trainerStats.totalHours, lastMonthStats.trainerStats.totalHours),
        changeType: this.calculateChangeType(currentStats.trainerStats.totalHours, lastMonthStats.trainerStats.totalHours),
        unit: 'hours',
      ...
      },
      {
        ...
        change: this.calculateChange(currentStats.memberStats.conversionRate, lastMonthStats.memberStats.conversionRate),
        changeType: this.calculateChangeType(currentStats.memberStats.conversionRate, lastMonthStats.memberStats.conversionRate),
        unit: '%',
  ...
  private groupRevenueByMonth(billing: any[]): Array<{ month: string; revenue: number }> {
    const monthlyRevenue = billing.reduce((acc, b) => {
      const month = new Date(b.period.startDate).toLocaleString('de-DE', { month: 'short', year: 'numeric' });
