...
export class EmailService implements IEmailService {
  ...
  static async sendBookingConfirmation(
    ...
    const formattedStart = formatDate(data.sessionStart);
    const formattedEnd = formatDate(data.sessionEnd);
    const template = templates.bookingConfirmation({
      ...data,
  ...
  static async sendBookingCancellation(
    to: string,
    data: { memberName: string; reason: string; sessionStartFormatted?: string; notes?: string }
  ): Promise<void> {
    const template = templates.bookingCancellation({
      memberName: data.memberName,
      reason: data.reason,
  ...
  static async sendBookingStatusChanged(
    ...
    const template = templates.bookingStatusChanged({
      memberName: data.memberName,
