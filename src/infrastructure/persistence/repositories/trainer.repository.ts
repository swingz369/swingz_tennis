...
export class DrizzleTrainerRepository implements TrainerRepository {
  ...
  async findByClub(clubId: ClubId): Promise<Trainer[]> {
    ...
    return trainersData.map((trainer: typeof trainers.$inferSelect) => {
      const clubIds = trainerClubsList
        .filter((tc: typeof trainerClubs.$inferSelect) => tc.trainer_id === trainer.id)
        .map((tc: typeof trainerClubs.$inferSelect) => tc.club_id)
  ...
  async findBySpecialty(specialty: string): Promise<Trainer[]> {
    ...
    return result.map((row: typeof trainers.$inferSelect) => this.mapToDomain(row));
  }
