...
-- Table pre-existed live under an earlier, incomplete definition (missing
-- most columns below) — backfill via ADD COLUMN IF NOT EXISTS so this is
-- safe whether news_posts is brand new or already there.
ALTER TABLE news_posts ADD COLUMN IF NOT EXISTS club_id uuid REFERENCES clubs(id) ON DELETE CASCADE;
ALTER TABLE news_posts ADD COLUMN IF NOT EXISTS title varchar(200);
ALTER TABLE news_posts ADD COLUMN IF NOT EXISTS slug varchar(250);
ALTER TABLE news_posts ADD COLUMN IF NOT EXISTS content text;
ALTER TABLE news_posts ADD COLUMN IF NOT EXISTS excerpt text;
ALTER TABLE news_posts ADD COLUMN IF NOT EXISTS cover_image_url text;
ALTER TABLE news_posts ADD COLUMN IF NOT EXISTS author_id uuid REFERENCES users(id) ON DELETE CASCADE;
ALTER TABLE news_posts ADD COLUMN IF NOT EXISTS category varchar(50) DEFAULT 'general';
ALTER TABLE news_posts ADD COLUMN IF NOT EXISTS status varchar(20) DEFAULT 'draft';
ALTER TABLE news_posts ADD COLUMN IF NOT EXISTS is_pinned boolean DEFAULT false;
ALTER TABLE news_posts ADD COLUMN IF NOT EXISTS published_at timestamp;
ALTER TABLE news_posts ADD COLUMN IF NOT EXISTS tags text[];
ALTER TABLE news_posts ADD COLUMN IF NOT EXISTS view_count integer DEFAULT 0;
ALTER TABLE news_posts ADD COLUMN IF NOT EXISTS created_at timestamp NOT NULL DEFAULT NOW();
...
ALTER TABLE news_comments ADD COLUMN IF NOT EXISTS news_post_id uuid REFERENCES news_posts(id) ON DELETE CASCADE;
ALTER TABLE news_comments ADD COLUMN IF NOT EXISTS user_id uuid REFERENCES users(id) ON DELETE CASCADE;
ALTER TABLE news_comments ADD COLUMN IF NOT EXISTS content text;
ALTER TABLE news_comments ADD COLUMN IF NOT EXISTS is_edited boolean DEFAULT false;
ALTER TABLE news_comments ADD COLUMN IF NOT EXISTS created_at timestamp NOT NULL DEFAULT NOW();
ALTER TABLE news_comments ADD COLUMN IF NOT EXISTS updated_at timestamp NOT NULL DEFAULT NOW();
...
-- News posts policies
DROP POLICY IF EXISTS "members_can_view_published_news" ON news_posts;
...
DROP POLICY IF EXISTS "admins_can_manage_news" ON news_posts;
...
DROP POLICY IF EXISTS "members_can_view_comments" ON news_comments;
...
DROP POLICY IF EXISTS "members_can_create_comments" ON news_comments;
...
DROP POLICY IF EXISTS "users_can_update_own_comments" ON news_comments;
CREATE POLICY "users_can_update_own_comments" ON news_comments
  FOR UPDATE USING (user_id = auth.uid());

DROP POLICY IF EXISTS "users_can_delete_own_comments" ON news_comments;
