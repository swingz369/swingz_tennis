...
DROP POLICY IF EXISTS "Members can view own attendance" ON attendance_records;
CREATE POLICY "Members can view own attendance" ON attendance_records FOR SELECT USING (participant_id = auth.uid());
DROP POLICY IF EXISTS "Trainers can view session attendance" ON attendance_records;
