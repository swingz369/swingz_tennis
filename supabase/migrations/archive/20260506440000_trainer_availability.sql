...
ALTER TABLE trainer_absences ADD COLUMN IF NOT EXISTS user_id uuid REFERENCES users(id) ON DELETE CASCADE;
ALTER TABLE trainer_absences ADD COLUMN IF NOT EXISTS club_id uuid REFERENCES clubs(id) ON DELETE CASCADE;
ALTER TABLE trainer_absences ADD COLUMN IF NOT EXISTS start_date date;
ALTER TABLE trainer_absences ADD COLUMN IF NOT EXISTS end_date date;
ALTER TABLE trainer_absences ADD COLUMN IF NOT EXISTS reason varchar(100);
ALTER TABLE trainer_absences ADD COLUMN IF NOT EXISTS notes text;
ALTER TABLE trainer_absences ADD COLUMN IF NOT EXISTS substitute_trainer_id uuid REFERENCES users(id) ON DELETE SET NULL;
ALTER TABLE trainer_absences ADD COLUMN IF NOT EXISTS created_at timestamp NOT NULL DEFAULT NOW();
ALTER TABLE trainer_absences ADD COLUMN IF NOT EXISTS updated_at timestamp NOT NULL DEFAULT NOW();
...
ALTER TABLE trainer_assignments ADD COLUMN IF NOT EXISTS user_id uuid REFERENCES users(id) ON DELETE CASCADE;
ALTER TABLE trainer_assignments ADD COLUMN IF NOT EXISTS club_id uuid REFERENCES clubs(id) ON DELETE CASCADE;
ALTER TABLE trainer_assignments ADD COLUMN IF NOT EXISTS hourly_rate numeric(10, 2) DEFAULT 0.00;
ALTER TABLE trainer_assignments ADD COLUMN IF NOT EXISTS specialization text[];
ALTER TABLE trainer_assignments ADD COLUMN IF NOT EXISTS max_students_per_session integer DEFAULT 10;
ALTER TABLE trainer_assignments ADD COLUMN IF NOT EXISTS bio text;
ALTER TABLE trainer_assignments ADD COLUMN IF NOT EXISTS qualifications text[];
ALTER TABLE trainer_assignments ADD COLUMN IF NOT EXISTS languages text[] DEFAULT ARRAY['de'];
ALTER TABLE trainer_assignments ADD COLUMN IF NOT EXISTS is_active boolean NOT NULL DEFAULT true;
ALTER TABLE trainer_assignments ADD COLUMN IF NOT EXISTS start_date date NOT NULL DEFAULT CURRENT_DATE;
ALTER TABLE trainer_assignments ADD COLUMN IF NOT EXISTS end_date date;
ALTER TABLE trainer_assignments ADD COLUMN IF NOT EXISTS created_at timestamp NOT NULL DEFAULT NOW();
ALTER TABLE trainer_assignments ADD COLUMN IF NOT EXISTS updated_at timestamp NOT NULL DEFAULT NOW();
...
DROP POLICY IF EXISTS "trainers_can_view_own_availability" ON trainer_availability;
...
DROP POLICY IF EXISTS "trainers_can_manage_own_availability" ON trainer_availability;
CREATE POLICY "trainers_can_manage_own_availability" ON trainer_availability
  FOR ALL USING (user_id = auth.uid());

DROP POLICY IF EXISTS "admins_can_manage_trainer_availability" ON trainer_availability;
...
DROP POLICY IF EXISTS "trainers_can_view_own_absences" ON trainer_absences;
...
DROP POLICY IF EXISTS "admins_can_manage_trainer_absences" ON trainer_absences;
...
DROP POLICY IF EXISTS "members_can_view_trainer_assignments" ON trainer_assignments;
...
DROP POLICY IF EXISTS "trainers_can_view_own_assignments" ON trainer_assignments;
CREATE POLICY "trainers_can_view_own_assignments" ON trainer_assignments
  FOR SELECT USING (user_id = auth.uid());

DROP POLICY IF EXISTS "admins_can_manage_trainer_assignments" ON trainer_assignments;
...
DROP TRIGGER IF EXISTS update_trainer_absences_updated_at ON trainer_absences;
CREATE TRIGGER update_trainer_absences_updated_at BEFORE UPDATE ON trainer_absences
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_trainer_assignments_updated_at ON trainer_assignments;
