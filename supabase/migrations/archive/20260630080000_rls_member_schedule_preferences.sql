...
DROP POLICY IF EXISTS member_schedule_prefs_select_own ON public.member_schedule_preferences;
CREATE POLICY member_schedule_prefs_select_own ON public.member_schedule_preferences
  FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS member_schedule_prefs_insert_own ON public.member_schedule_preferences;
CREATE POLICY member_schedule_prefs_insert_own ON public.member_schedule_preferences
  FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS member_schedule_prefs_update_own ON public.member_schedule_preferences;
CREATE POLICY member_schedule_prefs_update_own ON public.member_schedule_preferences
  FOR UPDATE USING (auth.uid() = user_id);

DROP POLICY IF EXISTS member_schedule_prefs_delete_own ON public.member_schedule_preferences;
