...
-- Indexes
CREATE INDEX IF NOT EXISTS idx_season_billing_configs_season ON season_billing_configs(season_id);
CREATE INDEX IF NOT EXISTS idx_season_billing_configs_club ON season_billing_configs(club_id);

-- RLS
ALTER TABLE season_billing_configs ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Club admins can manage billing configs" ON season_billing_configs;
...
DROP TRIGGER IF EXISTS trigger_season_billing_configs_updated_at ON season_billing_configs;
