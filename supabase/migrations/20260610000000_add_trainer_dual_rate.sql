...
COMMENT ON COLUMN public.trainer_profiles.contracted_hourly_rate IS
  'Contractually agreed hourly rate (admin-only write, read-only for trainer). Numeric(10,2), EUR. NULL = no contract rate set; in that case fall back to hourly_rate.';

COMMENT ON COLUMN public.trainer_profiles.extra_hours_rate IS
  'Hourly rate the trainer can freely set for additional hours they offer (e.g. extra sessions beyond the contract). Numeric(10,2), EUR. Editable by the trainer themselves; admin can also write.';
