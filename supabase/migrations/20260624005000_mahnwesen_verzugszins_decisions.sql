...
CREATE INDEX IF NOT EXISTS board_decisions_next_review_idx ON public.board_decisions (next_review)
  WHERE outcome = 'deferred'::public.decision_outcome;
...
DROP POLICY IF EXISTS board_decisions_admin_manage ON public.board_decisions;
...
DROP POLICY IF EXISTS board_decisions_member_read_completed ON public.board_decisions;
...
DROP POLICY IF EXISTS meeting_invitations_admin_manage ON public.meeting_invitations;
...
DROP POLICY IF EXISTS meeting_invitations_member_read_own ON public.meeting_invitations;
CREATE POLICY meeting_invitations_member_read_own ON public.meeting_invitations
  FOR SELECT TO authenticated
  USING (member_id = auth.uid());

DROP POLICY IF EXISTS meeting_invitations_member_update_own ON public.meeting_invitations;
...
DROP POLICY IF EXISTS decision_votes_voter_read_own ON public.decision_votes;
CREATE POLICY decision_votes_voter_read_own ON public.decision_votes
  FOR SELECT TO authenticated
  USING (voter_id = auth.uid());

DROP POLICY IF EXISTS decision_votes_voter_insert_own ON public.decision_votes;
CREATE POLICY decision_votes_voter_insert_own ON public.decision_votes
  FOR INSERT TO authenticated
  WITH CHECK (voter_id = auth.uid());

DROP POLICY IF EXISTS decision_votes_voter_update_own ON public.decision_votes;
CREATE POLICY decision_votes_voter_update_own ON public.decision_votes
  FOR UPDATE TO authenticated
  USING (voter_id = auth.uid());

DROP POLICY IF EXISTS decision_votes_admin_read_all ON public.decision_votes;
