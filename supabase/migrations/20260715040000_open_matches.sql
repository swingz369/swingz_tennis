...
DROP POLICY IF EXISTS open_matches_select ON open_matches;
...
DROP POLICY IF EXISTS open_matches_insert ON open_matches;
...
DROP POLICY IF EXISTS open_matches_update ON open_matches;
CREATE POLICY open_matches_update ON open_matches
  FOR UPDATE USING (auth.uid() = creator_id);

-- Creator can delete their own matches
DROP POLICY IF EXISTS open_matches_delete ON open_matches;
CREATE POLICY open_matches_delete ON open_matches
  FOR DELETE USING (auth.uid() = creator_id);

-- Participants: club members can view
DROP POLICY IF EXISTS open_match_participants_select ON open_match_participants;
...
DROP POLICY IF EXISTS open_match_participants_insert ON open_match_participants;
...
DROP POLICY IF EXISTS open_match_participants_update ON open_match_participants;
...
DROP TRIGGER IF EXISTS trigger_open_matches_updated_at ON open_matches;
